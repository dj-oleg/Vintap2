...
<div class="w-full">
                     <div data-fetch-key="data-v-1e870b96:0" class="pt-10" data-v-1e870b96>
                        <div class="mx-auto mb-16 max-w-[550px] sm:mb-28" data-v-1e870b96>
                           <h1 class="px-1 text-center text-[33px] font-medium tracking-[-1.2px] sm:px-6 sm:text-6xl" data-v-1e870b96>Browse selections</h1>
                        ...
                        <div id="selections_table" class="mt-10 mb-16 grid grid-cols-2 gap-5 sm:mb-28 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6" data-v-1e870b96>
                          ...
                          <?php foreach ($selections_g as $selection):?>
                            <?php // myCode Check Empty packs ?>                            
                            <?php  $checkPack = $myThis->getDatsPacks($this->context->num_genre($selection['genre'])); $flag='';
                                if (!$checkPack): $flag = 'none'; endif; ?>
                           <div style="display: <?php echo $flag; ?>" class="group relative overflow-hidden rounded-md border border-black/[0.12] dark:border-white/[0.12]" data-v-1e870b96>
                              <div class="absolute bottom-0 left-0 z-[1] flex w-full items-center p-3 sm:p-4" style="background: linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, rgba(0, 0, 0, 0.94) 71.35%)">
                                 <div class="flex-1 truncate">
                                    <a href="#" class="block truncate text-center text-sm font-bold tracking-[-0.2px] text-white sm:text-lg sm:!leading-6"><?=$selection['genre']?></a>
                               ...
                               <?php 
//                echo $this->context->num_genre($selection['genre']);
//                               var_dump($this->getDatsPacks(3));
//                                    var_dump($myThis->getDatsPacks($this->context->num_genre($selection['genre'])));
                              ...
                              <div class="pointer-events-none absolute inset-0 flex h-full w-full items-center justify-center bg-black bg-opacity-40 text-white transition-opacity focus:outline-none group-hover:opacity-100 opacity-0">
                                 <button audio_id="<?=$selection['audio_id']?>" class="pointer-events-auto rounded-full text-purple backdrop-blur-lg hover:text-lightpurple">
                         ...
                         <?php foreach ($selections_i as $selection):?>
                            
                             <?php 
                            $checkInst = $myThis->getDatsInst($this->context->num_instrument($selection['genre'])); $flagInst='';
                                if (!$checkInst){ $flagInst = 'none';}
                                else if ($this->context->num_instrument($selection['genre'])==0){$flagInst = 'none';}
                            
                            ?>
                            
                            
                            
                          <div style="display: <?php echo $flagInst; ?>" class="group relative overflow-hidden rounded-md border border-black/[0.12] dark:border-white/[0.12]" data-v-1e870b96>
                             <div class="absolute bottom-0 left-0 z-[1] flex w-full items-center p-3 sm:p-4" style="background: linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, rgba(0, 0, 0, 0.94) 71.35%)">
                                <div class="flex-1 truncate">
                                   <a href="#" class="block truncate text-center text-sm font-bold tracking-[-0.2px] text-white sm:text-lg sm:!leading-6"><?=$selection['genre']?></a>
                                   <p class="mt-0.5 truncate text-center text-[11px] leading-[14px] tracking-[0.1px] text-white/[0.72] dark:text-white/50 sm:mt-1"><?=$selection['creators']?></p>
                             ...
                             <a href="<?=Url::to(['site/samples'])?>?instruments=<?=$this->context->num_instrument($selection['genre'])?>" class="relative block h-0 w-full bg-cover bg-center pb-[84%] md:pb-[76%]" style="background-image:linear-gradient(0deg, rgba(0, 0, 0, 0.12), rgba(0, 0, 0, 0.12)), url(<?=Yii::$app->request->baseUrl?>/modx/<?=($selection['image']!="")?$selection['image']:'images/user_photo/user_icon.png'?>);"></a>
                             <div class="pointer-events-none absolute inset-0 flex h-full w-full items-center justify-center bg-black bg-opacity-40 text-white transition-opacity focus:outline-none group-hover:opacity-100 opacity-0">
                                <button audio_id="<?=$selection['audio_id']?>" class="pointer-events-auto rounded-full text-purple backdrop-blur-lg hover:text-lightpurple">
                                   <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-16" data-v-e4982a0e>
                                      <g data-v-e4982a0e>
