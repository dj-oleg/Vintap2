...
<div data-v-066bcea0="" data-v-f9b21242="">
   ...
   <div class="mt-8" data-v-066bcea0="">
      <div data-v-066bcea0="">
        <form id="uploadform" method="post" action="<?=Url::to(['user/loadfile'])?>">
          <input type="hidden" name="_csrf" value="<?=Yii::$app->request->getCsrfToken()?>" />
          <input type="hidden" name="type_file" value="<?=$type_file?>">
          <input type="hidden" name="id_pack" value="<?=$id_pack?>">
          
         <!--<input  type="file" class="dropZone" id="files" multiple="multiple" style="position:absolute;right:0;opacity:0;font-size:100px;height:311px">-->
         <div id="dropZone" class="dropZone group border border-dashed px-5 py-20 text-center hover:border-black dark:hover:border-white dz-clickable">
            ...
            <div class="dz-message">
               ...
               <p class="mt-8 text-black dark:text-white">Drag images to upload file(s) from your device!</p>
               <p class="mt-5 text-xs text-black opacity-30 dark:text-white">
                  Maximum upload size per file is 200MB.<br>
                  Image file format accepted only.
               </p>
            </div>
         </div>
       </form>
...
<?php
$url_load=Url::to(['user/loadfile2']);
...
for (var i = 0; i < event.dataTransfer.files.length; i++) {
        ...
        //xhr.send(file);
          ...
          formData.append("fileimgpr[]", file);
